...
def load_data_HDF(image_file, label_file):
    ...
    with h5py.File(image_file, 'r') as f_img:
        # 假设你的数据在 f_img['/image_data']，改成你真实的路径
        image_data = np.array(f_img['image_data'])
        # 如果是多维，需要转置或 reshape，依据 MATLAB 存储格式
        # 比如 MATLAB 是 (rows, cols, bands, samples)，你可能要变成 (samples, bands, rows, cols):
        # image_data = np.transpose(image_data, (3, 2, 0, 1))

    # 2. 读取标签数据
    with h5py.File(label_file, 'r') as f_lbl:
        # 同理，改成你真实的变量名，例如 'labels'
        gt = np.array(f_lbl['labels'])
        # 有时标签是 1×N 或 N×1，需要 flatten
        gt = gt.flatten()

    # 3. 对图像数据做标准化／缩放
    # 这里给个例子：每个波段减去均值，除以标准差
    # reshape 成 (samples, bands, -1) 先算统计量
    n_samples, n_bands, h, w = image_data.shape
    reshaped = image_data.reshape(n_samples, n_bands, -1)
    mean = reshaped.mean(axis=(0, 2), keepdims=True)
    std  = reshaped.std(axis=(0, 2), keepdims=True)
    scaled = (reshaped - mean) / (std + 1e-8)
    # 还原回原形状
    Data_Band_Scaler = scaled.reshape(n_samples, n_bands, h, w)

    return Data_Band_Scaler, gt
