...
# ============= Hard Negative Mining 对比学习损失 =============
# 三种策略可选: 'hard', 'semihard', 'weighted'
# hard_neg_weight: 困难负样本权重倍数 (推荐范围: 1.5-3.0)
# neg_sample_ratio: 负样本保留比例 (仅对'hard'策略有效，推荐: 0.3-0.7)
SupConLoss_HNM = loss_function.SupConLoss_HNM(
    temperature=0.1, 
    base_temperature=0.1,
    hard_neg_weight=2.0,          # 困难负样本权重
    neg_sample_ratio=0.5,         # 保留50%的困难负样本
    mining_strategy='weighted'    # 推荐使用weighted策略
).to(GPU)
...
contex_loss_fn = ConTeXLossAll(temperature=0.1, lambda_weight=1.0).to(GPU)
...
seeds = [1236, 1237, 1230, 1238, 1211, 1210, 1216, 1240, 1235, 1223]

# seeds = [1236, 1230, 1238]
...
for iDataSet in range(nDataSet):
    ...
    # encoder = mynet(in_channels=64, mask_classification=True, num_classes=TAR_CLASS_NUM, hidden_dim=49, num_queries=1, nheads=7, nheads_spa=8,
    #   dim_feedforward=49 * 2, dim_feedforward_spa=64*2, dec_layers=2, dec_layers_spa=0,  pre_norm=True, emb_size=emb_size, dropout=0.3).to(GPU)
    # encoder = mynet(in_channels=64, mask_classification=True, num_classes=TAR_CLASS_NUM, hidden_dim=49, num_queries=1, nheads=7,
    #   dim_feedforward=49 * 2, dec_layers=2, pre_norm=True, emb_size=emb_size, dropout=0.3).to(GPU)
    encoder = Net(inp_channels=N_DIMENSION, dim=emb_size, depths=[1], num_heads_spa=[8], num_heads_spe=[7], dropout=0.3).to(GPU)
    # encoder = Former(patch_size=patch_size, img_chans=N_DIMENSION ,emb_size=emb_size, dropout=0.3).to(GPU)
    # encoder = CSSTFormer(patch_size=patch_size, img_chans=N_DIMENSION ,emb_size=emb_size, num_heads=8, num_heads_spe=patch_size, dropout=0.1).to(GPU)
    # encoder = SSFE_net(n_dimension=N_DIMENSION, patch_size=patch_size, emb_size=emb_size, dropout=0.3).to(GPU)
    # Generator_network = PrototypeGenerator().to(GPU)
    ...
    for episode in range(EPISODE):                # 遍历所有训练轮次（元学习中的episode概念）
        ...
        augment1_target_ssl_data = torch.FloatTensor(
            data_augment.random_mask_batch_spatial(target_ssl_data.data.cpu(), 0.8))  # (64, 150, 7, 7) 116 5 113 1 113 2
        ...
        CTx_loss_tar = SupConLoss_HNM(augment_target_ssl_feature, target_ssl_label, None, GPU)
        ...
        # 总损失
        # loss = f_loss + 2 * CTx_loss_tar + 0.001 * loss_wd + 1 * loss_disc
        loss = f_loss + 2 * CTx_loss_tar + 0.001 * loss_wd + 1 * loss_disc
        ...
        if (episode + 1) % 500 == 0 or episode == 0:     # 定期测试评估
            with torch.no_grad():
                ...
                #     model_save_path = os.path.join(model_save_dir, f"GF5-1105_dataset{iDataSet+1}_{TAR_LSAMPLE_NUM_PER_CLASS}shot.pth")
                #     torch.save({
    ...
    #     # 保存t-SNE图（高分辨率）
    #     tsne_save_path = f"visualization/tsne_GF5-1105_{TAR_LSAMPLE_NUM_PER_CLASS}shot_dataset{iDataSet+1}.png"
    ...
    # # 保存误差热力图（高分辨率）
    # heatmap_save_path = f"visualization/error_heatmap_GF5-1105_{TAR_LSAMPLE_NUM_PER_CLASS}shot_dataset{iDataSet+1}.png"
...
# # 添加总标题
# plt.suptitle(f'Experimental Results - GF5-1105 ({TAR_LSAMPLE_NUM_PER_CLASS}-Shot Learning)', 
#             fontsize=22, fontweight='bold', y=0.98)
# plt.tight_layout(rect=[0, 0, 1, 0.96])

# # 保存综合统计图（高分辨率）
# summary_save_path = f"visualization/summary_GF5-1105_{TAR_LSAMPLE_NUM_PER_CLASS}shot.png"
...
#                          "classificationMap/GF5-1105_{}shot.png".format(TAR_LSAMPLE_NUM_PER_CLASS))
